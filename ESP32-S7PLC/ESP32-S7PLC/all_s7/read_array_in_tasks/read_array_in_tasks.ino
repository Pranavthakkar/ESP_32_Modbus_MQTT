/*  
 *  Reading large array with in task not in loop
 *  
 *  ARRAYS ARE FOR S7 1200 AND S7 200 COMBINED WITH THREE TASK AVAILABLE 
 *  
 *  ---  @PRANAV 25/8/2021  20:48
  ----------------------------------------------------------------------*/
...
IPAddress S7_1215(10, 0, 0, 180); // PLC Address S7 1215
...
void Task2code( void * pvParameters ) {
  ...
  for (;;) {
    ...
    Read_1200 = Client_1200.ReadArea(S7AreaDB, // We are requesting DB access
                                       3,        // DB Number = 1
                                       0,        // Start from byte N.0
                                       4000,     // We need "Size" bytes  Stricly put the bytes that has created in S7 DB if 11 there then put here only 11
                                       &MyBuffer);  // Put them into our target (Buffer or PDU)
