 ...
 *  
 *  Writing with all data types where mqtt command is recieved and send over to it
 *  working very good

    WITH MULTIPLE VALUES
   ---  @PRANAV 24/8/2021  22:32
  ----------------------------------------------------------------------*/
TaskHandle_t Task1;
TaskHandle_t Task2;
...
String real[] = {"T1", "T6", "T7", "T8", "T11"};
String doubleword[] = {"T2", "T5", "T15"};
String number[] = {"T3", "T9", "T10", "T13", "T18", "T19", "T20"};
...
void s7write( void * pvParameters ) {
  ...
  for (;;) {
    ...
    Serial.println("----------IN S7 WRITE -------------");

    if(obj != 0){
      
    /////////////======================== WRITING DATA TO PLC =======================/////////////////////

    for (uint8_t t = 1; t <= 20; t++) {        //Running the for getting all T1 - Tn
      String p = String(t);
      String local = "T" + p;

      String dict_key = myArr[t].key;

      uint8_t dict_byte = myArr[t].value;


      if (obj.containsKey(local)) {     // Check if json has that key

        for (uint8_t r = 0; r <= 4; r++) {          /// Always put the max value of String Array do not plus it
      ...
      Serial.println("NO MQTT DATA FOR S7");
    }
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("esp32pranav")) {
      ...
      // Subscribe
      client.subscribe("esp32/output");       //Subscribing the topic name here
    } else {
